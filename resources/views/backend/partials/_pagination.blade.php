                       ...
                       @foreach ($element as $page => $url)
                           <span wire:key="paginator-{{ $paginator->getPageName() }}-{{ $this->numberOfPaginatorsRendered[$paginator->getPageName()] }}-page{{ $page }}">
                                   ...
                                   <span aria-current="page">
                                                <button  type="button"  class=" paginate_button  current">{{ $page }}</button>
                                            </span>
                               @else
                                   <button  type="button" wire:click="gotoPage({{ $page }}, '{{ $paginator->getPageName() }}')" class="paginate_button" aria-label="{{ __('Go to page :page', ['page' => $page]) }}">
                                                {{ $page }}
                                            </button>
                               @endif
                                    </span>
